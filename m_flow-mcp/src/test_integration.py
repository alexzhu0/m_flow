...
from learn_response_utils import is_api_mode_learn_response
...
"""
    ...
    async def test_api_mode_handling(self):
        ...
        try:
            async with self.session() as sess:
                # 测试 learn 工具（API 模式下应返回明确的成功或限制提示）
                print("\n  1️⃣  测试 learn 工具...")
                result = await sess.call_tool("learn", arguments={})
                content = result.content[0].text if result.content else ""

                if not is_api_mode_learn_response(content):
                    raise Exception(f"API 模式 learn 返回缺少明确模式信息: {content}")
